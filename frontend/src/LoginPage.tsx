        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 my-5">
              ...
              <span className="text-[10px] uppercase tracking-[0.2em] text-slate-500">або</span>
              <span className="h-px flex-1 bg-white/10" aria-hidden />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-left">
              {mode === 'register' && (
                <div>
                  <label className="block text-xs text-slate-400 mb-1.5">Ім’я (необов’язково)</label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2 text-sm text-white
                               placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-surface"
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full mt-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl
                           ...
                           transition-all duration-200 cursor-pointer"
              >
                {submitting ? 'Зачекайте...' : mode === 'login' ? 'Увійти' : 'Зареєструватись'}
              </button>
            </form>
          </>
        )}
